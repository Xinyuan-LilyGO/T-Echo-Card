 ...
 * @LastEditTime: 2026-01-02 17:54:12
 * @License: GPL 3.0
 */
...
#define MAX_IIS_DATA_TRANSMIT_SIZE 80 * MAX_IIS_DATA_TRANSMIT_COUNT * 5

#define MAX_IIS_STREAM_SIZE 512
#define MAX_IIS_STREAM_BLOCK_COUNT 5

#define MAX_IIS_TX_BUFFER_COUNT 3

#define MAX_PDM_DATA_TRANSMIT_SIZE 512
#define MAX_PDM_DATA_TRANSMIT_BLOCK_COUNT 5

#define MAX_CODEC2_ENCODE_BUFFER_COUNT 5
#define MAX_CODEC2_DECODE_BUFFER_COUNT 5
...
{
    Serial.println("Codec2_Task start");
    ...
    const auto sample_8khz_int16_t_size = codec2_samples_per_frame(Codec2_Status); // 8kHz下每帧的int16_t数据
    ...
    printf("codec2 config: 8k samples/frame: %d\nCodec2_Encode_Size: %d\nIis_Data_Transmit_Size: %d\n", Sample_8khz_Int8_t_Size, Codec2_Encode_Size, Iis_Data_Transmit_Size);
    ...
    while (1)
    {
        if (Codec2_Decode_Buffer.size() < MAX_CODEC2_DECODE_BUFFER_COUNT)
        {
            if (Codec2_Encode_Buffer.empty() == false)
            {
                ...
                // 解码 (还原) - 得到8kHz数据
                codec2_decode(Codec2_Status, codec2_decode_buffer.get(), codec2_encode_buffer.get());
...
void setup()
{
    ...
    xTaskCreate(&Pdm_Task, "Pdm_Task", 1 * 1024, NULL, 5, NULL);
...
void loop()
{
    ...
    {
        printf("Pdm_Stream size: %d\nCodec2_Encode_Buffer: %d\nCodec2_Decode_Buffer: %d\n", Pdm_Stream.size(), Codec2_Encode_Buffer.size(), Codec2_Decode_Buffer.size());
